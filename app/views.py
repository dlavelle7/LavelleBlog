...
Views are the handlers that respond to requests from web browsers.Views are written as Python functions. Each view is mapped to a URL
"""
# Import render_template to render template (e.g. index.html).
# Import g global (g implemented in 'before_request()' function).
from flask import render_template, flash, redirect, session, url_for, request, g
from flask.ext.login import login_user, logout_user, current_user, login_required
from app import app, db, lm, oid
# Import LoginForm class (to create an object and send to template)
...
def before_request():
    ...
    @app.before_request decorator, runs function before the view function each time a request is recieved. Good place to setup g.user variable.
    """
...
# Route decorators (@app.route) map to URLs '/' and '/index'
@app.route('/', methods = ['GET', 'POST'])
# Forms submitted to this function via POST
...
def index(page = 1):
    ...
    @login_required decorator requires user to be logged in before they can view this URL. Flask-Login will redirect user to login function if not already logged in (configured in __init__.py).
    """
    form = PostForm()
...
# 'methods' argument in the route decorator tells flask that this 
...
def login():
    ...
    return render_template('login.html', 
        title = 'Sign In',
	# Send LoginForm object to the template
        form = form,
	# Grab the openid configuration from config
...
def after_login(resp):
    ...
    If authentication is successful, Flask-OpenID will call a function registered with the @oid.after_login decorator.
    """
...
def search():
    ...
    Form sent here from action in search form (base.html). Search process not done here, redirected to search_results function
    """
